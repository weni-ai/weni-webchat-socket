...
- Q: `research.md` §1 and `contracts/audiosocket-session-protocol.md` §2 previously labeled AudioSocket byte `0x03` as "Error" in the frame-type table. This spec's Out of Scope section already excludes IVR/DTMF handling, and the Product Spec never defines these byte values — is this a Product Spec defect or an engineering-artifact defect, and what is the correct mapping? → A: **Engineering-artifact defect only** — the Product Spec never specifies AudioSocket byte values (correctly deferred as an implementation detail), so it required no change. `research.md`/`contracts` mis-transcribed Asterisk's own `ast_audiosocket_msg_kind` enum: the correct mapping is `0x00` hangup, `0x01` UUID, `0x03` **DTMF** (not error), `0x10` audio, `0xFF` **Error** (not `0x03`). Both artifacts are corrected accordingly; `0x03`/DTMF frames are recognized-and-ignored per the existing IVR/DTMF out-of-scope boundary, never logged/metriced as errors.
- Q: `research.md` §5, `contracts/grpc-telephony-delivery.md` §2, and `tasks.md` T043 previously described telephony's `Router`/`DeliverFunc` wiring as "extending" or adding an "if branch" to the existing `api` process's `Router`/`ClientPool`, implying a shared delivery table. Given `pkg/streams/router.go`'s `Router` and `pkg/websocket`'s `ClientPool` are pod-local, is that model accurate, and does implementing it as described risk touching `pkg/websocket` (violating BD-009)? → A: **The "shared Router with an if branch" model was inaccurate — corrected.** Verified against `grpc/main.go` and `pkg/websocket/router_factory.go`: every existing binary (`api`, `grpc`) already constructs its **own** `streams.Router` instance, bound to its **own** `podID`, with its **own** `deliver`/`isLocal`/`lookup` closures — none are shared (`grpc/main.go`'s is publish-only, `isLocal` always `false`). `telephony/main.go` MUST follow this same established pattern: construct its own `Router` (own `podID`) whose `deliver` closure looks up `SessionManager.GetByRegistrationKey` directly. No `if` branch or fallthrough to `ClientPool` is needed *or* correct, because `Router.PublishToClient` already resolves the target pod from `ConnectedClient.PodID` before ever writing to a stream — a `CallSession` (registered under the telephony pod's own `PodID`) only ever receives stream entries on the telephony pod's own `Router`. This requires zero changes to `pkg/streams/router.go` **or** `pkg/websocket/router_factory.go`; the corrected model actually reduces BD-009 risk versus the original wording, which — if implemented literally — could have led to an unnecessary edit inside `pkg/websocket`.
