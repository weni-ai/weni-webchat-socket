...
- Q: How does a Nexus response delta reach the right phone call instead of a WebSocket client? → A: Reuse the existing `ClientManager` + `Router` (Redis Streams) abstractions unchanged. A `CallSession` registers itself as a `ConnectedClient` keyed by the contact's `tel:` URN; the existing gRPC `StreamMessages`/`SendMessage` code path in `pkg/grpc/server.go` requires no changes — it already delivers by contact URN, agnostic of what "connected" means. See `research.md` §3.
...
2. **Given** a `CallSession` is active, **When** it starts, **Then** the gateway registers it in `ClientManager`/`Router` keyed by the contact's `tel:` URN, exactly as a WebSocket client would register, so existing gRPC delivery code requires no changes (implements Product FR-013 delivery path).
...
- **FR-014**: The gateway MUST register each active `CallSession` in the existing `ClientManager`/`Router` as a connected client keyed by the contact's `tel:` URN, so the existing gRPC `MessageStreamService` delivers Nexus response deltas to it without any change to the gRPC server code. (Story 3, Scenarios 2–3; implements Product FR-013 delivery path.)
