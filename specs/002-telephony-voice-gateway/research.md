...
1. Registers itself via `ClientManager.AddConnectedClient(ConnectedClient{ID: contactURN, Channel: channelUUID, PodID: podID})` exactly as a WebSocket client does today.
2. Supplies its own `DeliverFunc` implementation (routed into the pod-local delivery table alongside WebSocket clients) that parses the same JSON stream payloads (`StreamStartPayload`/`StreamDeltaPayload`/`StreamEndPayload` — already defined in `pkg/websocket`) and feeds them into the TTS batching pipeline (Story 4) instead of writing to a WebSocket connection.
3. Deregisters via `ClientManager.RemoveConnectedClient` on teardown.
